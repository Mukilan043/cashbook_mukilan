...
const CashInflow = ({ cashbookId, onDone }) => {
  ...
  const [receiptType, setReceiptType] = useState('inflow');
  ...
  const handleScanReceipt = async () => {
    ...
    try {
      ...
      setReceiptType(result.type || 'inflow');
      ...
      if (!hasInflow && hasOutflow) {
        setScanError('This receipt looks like an outflow. Please add it in Cash Outflow.');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Cash inflow added successfully!' });
      setFormData({
        amount: '',
        description: '',
        date: format(new Date(), 'yyyy-MM-dd'),
      ...
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.error || 'Failed to add cash inflow' 
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-md mx-auto cd-card" style={{ padding: '24px' }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div
          className={`rounded-lg border border-dashed px-4 py-3 ${dragActive ? 'border-green-500 bg-green-100 ring-2 ring-green-200' : 'border-green-300 bg-green-50'}`}
        ...
        >
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-sm font-semibold text-green-700">If you have Receipt,please Upload Receipt</div>
                <div className="text-xs text-green-600">Drag and drop, or upload (JPG, PNG, PDF)</div>
              </div>
              <label className="inline-flex items-center gap-2 rounded-md bg-white px-3 py-2 text-xs font-semibold text-green-700 shadow cursor-pointer">
                <input
                  type="file"
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center rounded-md bg-green-600 px-3 py-2 text-xs font-semibold text-white shadow hover:bg-green-700 disabled:opacity-60"
            >
                  ...
                  return (
                    <div key={key} className="grid grid-cols-7 gap-2 items-center">
                      <input
                        ...
                        onChange={(e) => handleReceiptItemChange(index, 'name', e.target.value)}
                        readOnly={!editingItems.includes(key)}
                        className="col-span-4 w-full rounded-md border border-red-200 px-2 py-1 text-sm"
                        placeholder="Item name"
                      />
                      <input
                        type="number"
                        ...
                        onChange={(e) => handleReceiptItemChange(index, 'amount', e.target.value)}
                        readOnly={!editingItems.includes(key)}
                        className="col-span-2 w-full rounded-md border border-red-200 px-2 py-1 text-sm"
                        placeholder="Amount"
                      />
                      <div className="relative col-span-1 flex justify-end">
                        <button
        ...
        >
          {loading ? 'Adding...' : 'Add Cash Inflow'}
        </button>
      </form>
    </div>
  );
